...
function App() {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`http://localhost:5000/api/parental-legacy?day=${selectedDate}`)
      .then((res) => res.json())
      .then((json) => {
        setData(json);
        setLoading(false);
  ...
  if (loading) return <div className="min-h-screen bg-[#080a10] text-white flex items-center justify-center font-black italic tracking-tighter text-2xl">LOADING BIOMETRICS...</div>;
  
  if (!data) return <div className="min-h-screen bg-[#080a10] text-red-500 flex items-center justify-center font-bold">⚠️ SERVER OFFLINE: Please run 'node server.js'</div>;
      ...
      <div className="max-w-6xl mx-auto bg-slate-900/60 p-6 md:p-8 rounded-[2rem] md:rounded-[3rem] border border-white/5 shadow-2xl">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-6 mb-10">
          <div>
            ...
            <p className="text-[10px] text-slate-500 font-black tracking-[0.3em] mt-1">API</p>
          </div>

          <div className="flex items-center gap-4">
            <div className={`flex items-center gap-2 px-4 py-2 rounded-full border ${isMotherDominant ? 'border-pink-500/50 bg-pink-500/10' : 'border-blue-500/50 bg-blue-500/10'}`}>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="text-slate-500 text-[10px] font-black uppercase border-b border-white/10">
                ...
                <th className={`p-4 text-center ${isMotherDominant ? 'text-pink-400 bg-pink-400/5' : ''}`}>Mother</th>
                <th className={`p-4 text-center ${!isMotherDominant ? 'text-blue-400 bg-blue-400/5' : ''}`}>Father</th>
            ...
            <tbody className="divide-y divide-white/5">
              {factors.map((row, i) => (
                <tr key={i} className="hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className={`p-4 text-center font-mono text-lg md:text-xl ${isMotherDominant ? 'text-pink-300 font-black' : 'text-pink-300/60'}`}>
                    {row.mother.toFixed(3)}
                  </td>
                  <td className={`p-4 text-center font-mono text-lg md:text-xl ${!isMotherDominant ? 'text-blue-300 font-black' : 'text-blue-300/60'}`}>
                    {row.father.toFixed(3)}
                  </td>
                  <td className="p-4 text-center font-mono font-black text-white bg-indigo-500/5">
                    {row.total.toFixed(3)}
                  </td>
            ...
            <tfoot>
              <tr className="bg-indigo-900/40 text-white font-black border-t-2 border-indigo-500/30">
                ...
                <td className="p-6 text-center text-pink-500 text-xl md:text-2xl">{mTotal.toFixed(3)}</td>
                <td className="p-6 text-center text-blue-500 text-xl md:text-2xl">{fTotal.toFixed(3)}</td>
                <td className="p-6 text-center bg-indigo-600 text-3xl md:text-4xl shadow-inner">
